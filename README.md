...
Before apply, patches are **normalized** (e.g. blank lines inside hunks are turned into single-space context lines) and `git apply` runs with **`--recount --inaccurate-eof`**, then retries with **`--ignore-space-change`** if the first attempt fails. On success it creates a **local** branch and **one commit** (same skip flags as below). Jira comment includes apply status, branch name, short commit hash, and the diff (truncated). Requires `git` on the server `PATH`, and the target repo should have `user.name` / `user.email` configured for commits.
